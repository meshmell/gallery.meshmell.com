...
const Sponsors = ({ lang, setModalOpen, setHoverOnModal, modalOpen }: SponsorsType) => {
  ...
  return (
    <>
      ...
      <div
        className={`transition-transform duration-150 rounded-lg z-[100] fixed bottom-[0px] sm:top-[0px] right-0 bg-neutral-100 dark:bg-neutral-950 p-6 w-full sm:w-[600px] h-[700px] sm:h-screen flex flex-col gap-4 ${modalOpen.sponsors ? "translate-y-0 sm:translate-y-0 translate-x-0 sm:translate-x-0 ease-in" : "translate-y-full sm:translate-y-[0px] -translate-x-[0px] sm:translate-x-[600px] ease-out"}`}
      ...
      >
        <div className="flex justify-end mb-4">
          <div onClick={handleClickClose} className={"flex justify-center items-center w-12 h-12 sm:w-14 sm:h-14 bg-transparent border-[2.2px] sm:border-[3px] border-black dark:border-white  rounded-full"}>
            <button className="text-base sm:text-xl font-bold text-black dark:text-white"><ImCross /></button>
          </div>
        </div>
        <div className="text-white">
          <h2 className={`font-bold ${lang === "en" ? "text-3xl sm:text-4xl" : "text-2xl sm:text-3xl"} mb-8 text-center text-black dark:text-white`}>{t("sponsors.thankTo_1")}<br />{t("sponsors.thankTo_2")}</h2>
