  ...
  const handleDownloadClick = () => {
    ...
    handleDownloadFileFromGCS(focusedModelsObj, resolution, isZipped)
      .then(() => {
        ...
        handleIncrementDownloadToFirebase(setFocusedModelsDownloadData, focusedModelsObj, setGetFirebaseDataLoading)
          ...
          {focusedModelsObj.isDownloadable ?
              ...
              <div className="flex flex-row justify-evenly gap-1 items-center">
                  ...
                  <div className="flex flex-col items-center">
                    {focusedModelsObj.resolutions.map((resolution, index) => (
                      <button
                        ...
                        className={`my-1 py-1 px-2 rounded border ${currentResolution === resolution
                  ...
                  {fileFormats.filter(format => focusedModelsObj.formats.includes(format.extension)).map((format, index) => (
                    <div key={index} className="flex items-center mb-2">
                      <input
                        ...
                        className="mr-2"
                      />
                      <label htmlFor={`format-${format.extension}`} className="cursor-pointer">
                ...
                <div className="w-12 sm:w-14 h-12 sm:h-14 flex justify-center items-center p-1 rounded-full border-2 border-black dark:border-white cursor-pointer">
                  {isDownloading ?
                    ...
                    :
                    <BiSolidDownload className="text-3xl sm:text-4xl" onClick={handleDownloadClick} />
                  }
                </div>
              </div>
            ...
            <div className="text-lg text-center">
              ...
              <div className="text-lg text-center mt-[15px]">
                <a href={focusedModelsObj.source?.downloadSite} className="text-blue-500 dark:text-blue-400 font-bold">
