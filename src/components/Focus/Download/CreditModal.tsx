...
const DownloadCreditModal = ({ lang, setModalOpen, focusedModelsCreatorsObj, isFocusedMode }: DownloadCreditModalType) => {
    ...
    <div className="fixed inset-0 bg-black dark:bg-gray-500 bg-opacity-50 dark:bg-opacity-50 z-[110] flex justify-center items-center">
      <div className="bg-neutral-100 dark:bg-neutral-900 p-6 w-96 rounded-lg">
        <div className="flex flex-col gap-4">
          ...
          <div className="text-xl sm:text-2xl font-bold">{t("download.thankTo")}</div>
          <CreatorInfo creatorsObj={focusedModelsCreatorsObj} lang={lang} isFocusedMode={isFocusedMode} />
        </div>
      </div>
    </div>
  )
