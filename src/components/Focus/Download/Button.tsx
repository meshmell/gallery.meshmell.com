...
const DownloadButton = ({ setModalOpen, modalOpen, getFirebaseDataLoading, focusedModelsDownloadData }: DownloadButtonType) => {
  ...
  return (
    <div className={`${modalOpen.download ? "bg-blue-500 border-blue-500 dark:border-blue-500 text-white" : "bg-neutral-100 dark:bg-neutral-950 text-black dark:text-white border-black dark:border-white"} cursor-pointer shadow-lg mt-2 mb-2 flex flex-col justify-center items-center rounded-full w-12 h-16 sm:w-14 sm:h-20 border-[1.5px] sm:border-[3px]`} onClick={handleClick}>
