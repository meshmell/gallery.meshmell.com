...
export const CameraControllerHorizontal = ({
  camera,
  enabled,
  theNumberOfModel,
  ...
  const columnNumberOfWindow =
    views.find((view) => view.slug === "horizontal")?.windowWidths[windowType]
      .columnNumberOfWindow ?? 0;
  const stepSize =
    views.find((view) => view.slug === "horizontal")?.windowWidths[windowType]
      .stepSize ?? 0;
  const horizontalView = views.find((view) => view.slug === "horizontal");
  const maxZ = horizontalView
    ? horizontalView.windowWidths[windowType].cameraStatusForList.position[2]
    : 0;
  ...
  const updateCameraPosition = (newPositionZ: number) => {
    ...
    gsap
      ...
      .then(() => {
        ...
        if (
          camera.position.x !==
          horizontalView?.windowWidths[windowType].cameraStatusForList
            .position[0]
        )
          return;
