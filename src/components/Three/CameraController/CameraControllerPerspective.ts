...
export const CameraControllerPerspective = ({ camera, enabled, theNumberOfModel, setSavedCameraStatus, windowType, isFocusedMode }: CameraControllerType) => {
  ...
  const cameraPositionYForTheEndView = perspectiveView?.windowWidths[windowType].cameraPositionYForTheEndView ?? 0;
  const minZ = -(stepSize) * (toNextEven(theNumberOfModel) / 2 - 1) + maxZ - cameraPositionYForTheEndView
  let startTouch = 0;
