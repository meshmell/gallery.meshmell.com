...
const ShareModalWhenList = ({ lang, setModalOpen, modalOpen, isFocusedMode }: ShareModalWhenListType) => {
  ...
  return (
    <>
      ...
      <div
        className={`transition-transform duration-150 rounded-lg z-[100] fixed bottom-[0px] sm:top-[0px] left-0 bg-neutral-100 dark:bg-neutral-950 p-6 w-full sm:w-[384px] h-[700px] sm:h-screen flex flex-col gap-4 ${modalOpen.shareThisPageInList ? "translate-y-0 sm:translate-y-0 translate-x-0 sm:translate-x-0 ease-in" : "translate-y-full sm:translate-y-[0px] -translate-x-[0px] sm:-translate-x-full ease-out"}`} onClick={handleClickInside}
      >
